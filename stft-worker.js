/**
 * Worker for the per-chunk STFT (pre) and iSTFT (post). Runs off the main
 * thread so it can overlap with the next chunk's GPU inference.
 *
 * Messages:
 *   { id, type: 'stft',         planarBuf, nFft, hopLength, winLength }
 *      -> { id, type: 'stft',         realBuf, imagBuf, F, T, elapsedMs }
 *   { id, type: 'istft-batch',  realBuf, imagBuf, numStems, numChannels,
 *                               F, T, length, nFft, hopLength, winLength }
 *      -> { id, type: 'istft-batch',  audioBuf, numStems, numChannels, length, elapsedMs }
 *
...
function stftStereo(planarAudio, { nFft, hopLength, winLength }) {
...
const cache = new Map();
function getCached(nFft, T, hopLength, winLength) {
...
function runStft(msg) {
  const t0 = performance.now();
  const planar = new Float32Array(msg.planarBuf);
  const { real, imag, F, T } = stftStereo(planar, {
    nFft: msg.nFft, hopLength: msg.hopLength, winLength: msg.winLength,
  });
  return {
    id: msg.id, type: 'stft',
    realBuf: real.buffer, imagBuf: imag.buffer,
    F, T,
    elapsedMs: performance.now() - t0,
  };
}
...
self.addEventListener('message', e => {
  ...
  if (msg.type === 'istft-batch') {
    ...
    self.postMessage(reply, [reply.audioBuf]);
  } else if (msg.type === 'stft') {
    const reply = runStft(msg);
    self.postMessage(reply, [reply.realBuf, reply.imagBuf]);
  }
});
